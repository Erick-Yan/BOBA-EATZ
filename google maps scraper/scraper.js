...
import bobaShops from '../data/boba_shops.json' assert {type: 'json'};
import bobaDrinks from '../data/boba_drinks.json' assert {type: 'json'};
...
const url = "http://google.com/maps/search/469+Queen+St+W+Toronto,+ON+M5V+2A9+Bubble+Lee/?h1=en"
const {executablePath} = require('puppeteer')

// Extract shop data
async function ExtractShopObjects() {
    let shopInfo = {};

    bobaShops.forEach(shop => {
        shopInfo[shop["shopName"]] = {
            shopAddress: shop["shopAddress"],
            mapsUrl: "http://google.com/maps/search/" 
                        + String(shop["shopAddress"] + " " + shop["shopName"]).split(' ').join("+")
                        + "/?h1=en",
            drinks: [],
        }
    });
    ...
    bobaDrinks.forEach(drink => {
        shopInfo[drink["shopName"]].drinks.push(drink["drinkName"]);
    });
...
async function getLocalPlaceReviews() {
    const browser = await puppeteer.launch({
        ...
        executablePath: './chrome-win/chrome-win'
    });
